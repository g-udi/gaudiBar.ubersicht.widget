...
export const WIDGET_DIR = 'gaudiBar.ubersicht.widget'
export const LEGACY_WIDGET_DIR = 'gaudiBar.widget'
export const widgetPath = (path = '') => `${WIDGET_DIR}${path ? `/${path}` : ''}`
...
export const withLocalEnv = (command) =>
    `GAUDI_BAR_WIDGET_DIR=${shellQuote(WIDGET_DIR)}; ` +
    `if [ ! -d "$GAUDI_BAR_WIDGET_DIR" ] && [ -d ${shellQuote(LEGACY_WIDGET_DIR)} ]; then GAUDI_BAR_WIDGET_DIR=${shellQuote(LEGACY_WIDGET_DIR)}; fi; ` +
    `export GAUDI_BAR_WIDGET_DIR; ` +
    `[ -f "$GAUDI_BAR_WIDGET_DIR/.env" ] && set -a && . "$GAUDI_BAR_WIDGET_DIR/.env" && set +a; ${command}`

export const runWithLocalEnv = (command) => Uebersicht.run(withLocalEnv(command))
